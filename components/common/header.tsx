...
const LogOutButton = dynamic(() => import('../adminDashboard/logOut'))
...
const Header = ({
  ...
  useEffect(() => {
    const Counts = notifications?.filter(
      (notify: any) => notify?.status === "pending",
    ).length;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          >
            ...
            <div>
              <h1 className="text-xl font-bold text-teal-700">Employee Hub</h1>
          ...
          <div className="flex items-center gap-6">
            {isAdmin && (
              <Link className="relative" href={"/admin-notification"}>
                <Bell className="w-7 h-7  text-teal-500" />
                {notifyCount !== 0 && (
                  <span
                    className="text-gray-100 font-bold text-[11px] w-5  h-5 bg-green-900
              flex items-center justify-center 
              rounded-full  absolute -top-1 left-3 "
                  >
                    {notifyCount}
                  </span>
                )}
