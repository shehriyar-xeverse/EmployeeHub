"use client"
import { Bell, Building2, CheckCircle, Clock, DollarSign, Mail, X } from 'lucide-react';
import Image from 'next/image';
import { Spinner } from '../ui/spinner';

const CardContent = ({filteredNotifications,handleApprove,handleReject,loading,rejectLoading}:any) => {
    ...
    return name.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2);
      ...
      const statusConfig: any = {
        pending: { color: 'text-yellow-400', bg: 'bg-yellow-500/10', border: 'border-yellow-500/20', badge: 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' },
        accept: { color: 'text-green-400', bg: 'bg-green-500/10', border: 'border-green-500/20', badge: 'bg-green-500/10 text-green-400 border-green-500/20' },
        reject: { color: 'text-red-400', bg: 'bg-red-500/10', border: 'border-red-500/20', badge: 'bg-red-500/10 text-red-400 border-red-500/20' }
      };
    ...
    return (
        ...
        {filteredNotifications?.map((notif: any) => {
          ...
          const Icon = statusIcons[status] || <Bell className="w-4 h-4 text-purple-400" />;
          ...
          return(
            <div
              ...
              className={`group bg-gradient-to-br from-[#1a1a1a] to-[#121212] rounded-2xl border ${
                status === 'pending' ? 'border-purple-500/30 shadow-lg shadow-purple-500/5' : 'border-gray-800/50'
              ...
              <div className="flex flex-col gap-4">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    {status === 'pending' && <span className="w-2 h-2 bg-purple-500 rounded-full animate-pulse"></span>}
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-4 bg-[#0a0a0a] rounded-xl border border-gray-800/50">
                  ...
                  <div className="flex items-center gap-3 col-span-1">
                    <div className="relative flex-shrink-0">
                      {notif?.profile_image ? (
                        ...
                        <div className="w-14 h-14 rounded-full bg-gradient-to-br from-purple-600 to-purple-800 flex items-center justify-center text-white font-bold text-lg border-2 border-gray-700">
                          {getInitials(notif?.name)}
                        </div>
                      )}
                      <div className={`absolute -bottom-0.5 -right-0.5 p-0.5 rounded-full ${config.bg} border-2 border-[#0a0a0a]`}>
                        {Icon}
                      </div>
                    </div>
                    <div>
                      <p className="text-xs text-gray-500">Name</p>
                      <p className="text-sm font-medium text-white truncate">{notif?.name || 'N/A'}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-white truncate">{notif?.email || 'N/A'}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-white truncate">{notif?.department || 'N/A'}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-white truncate">${notif?.salary || 'N/A'}</p>
                    </div>
                  </div>
                </div>
